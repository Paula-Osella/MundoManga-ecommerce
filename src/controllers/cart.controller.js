...
updateProdQuantityToCart: async (req, res) => {
  ...
  try {
    ...
    const updatedCart = await cartService.updateProdQuantityToCart(cartId, prodId, qty);
    return res.status(200).json({ message: "Cantidad del producto actualizada", updatedCart });
  } catch (error) {
    return res.status(500).json({ message: "Error al actualizar la cantidad", error: error.message });
  }
},
