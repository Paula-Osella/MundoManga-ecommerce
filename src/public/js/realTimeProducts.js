...
productsList.addEventListener("click", (e) => {
    ...
    socket.on("delete-product", async (data) => {
        try {
            const result = await productManager.deleteProduct(Number(data.id));
      ...
      <li id="product-${product.id}">
        <div class="product-summary">
          ...
          <div class="product-details" id="details-${product.id
            ...
            <button class="close-details" data-id="${product.id
            }">Cerrar Detalle</button>
          </div>

          <div class="product-update" id="update-${product.id
            }" style="display:none;">
            <form id="form-update-${product.id}" data-id="${product.id}">
              <label for="update-category-${product.id}">Categoría</label>
              <input type="text" id="update-category-${product.id}" value="${product.category
            }" required />

<label for="update-code-${product.id}">Código del Producto</label>
<input type="text" id="update-code-${product.id}" value="${product.code
            }" required />

<label for="update-title-${product.id}">Título</label>
<input type="text" id="update-title-${product.id}" value="${product.title
            }" required />

              <label for="update-description-${product.id}">Descripción</label>
              <textarea id="update-description-${product.id}" required>${product.description
            }</textarea>

              <label for="update-price-${product.id}">Precio</label>
              <input type="number" id="update-price-${product.id}" value="${product.price
            }" required />

              <label for="update-stock-${product.id}">Stock</label>
              <input type="number" id="update-stock-${product.id}" value="${product.stock
            }" required />

              <label for="update-status-${product.id}">Estado</label>
              <input type="checkbox" id="update-status-${product.id}" ${product.status ? "checked" : ""
            } />
